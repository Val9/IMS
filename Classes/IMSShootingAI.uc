...
class IMSShootingAI extends AIController;
...
function GetEnemy() // Find something to shoot at, yes that means you!
{
	if(Enemy == none)
 		{
		if(IMSPlayerController(WorldInfo.GetALocalPlayerController()) != none && IMSPlayerController(WorldInfo.GetALocalPlayerController()).Pawn != none)
		{
...
Begin:
	...
	MoveToward(Enemy, Enemy, 450, true);

		if (VSize(Pawn.Location - Enemy.Location) <= 550)
			GotoState('Shooting');
...
if(Enemy != none)
{
	`log(VSize(Pawn.Location - Enemy.Location));
